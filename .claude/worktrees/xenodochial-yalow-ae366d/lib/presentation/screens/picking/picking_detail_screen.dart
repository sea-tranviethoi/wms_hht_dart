...
class _PickingDetailScreenState extends State<PickingDetailScreen> {
  ...
  Future<bool?> _showConfirmDialog(String message) {
    return showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        title: const Text('確認', style: TextStyle(fontFamily: 'MSPGothic')),
  ...
  void _showSnack(String msg, {bool isError = false}) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      backgroundColor: isError ? AppColors.textError : AppColors.btnGreen,
  ...
  Widget build(BuildContext context) {
    return BlocListener<PickingBloc, PickingState>(
      ...
      child: _lines.isEmpty
          ? Scaffold(body: AppEmpty.list(message: 'データがありません'))
          : _buildScaffold(),
    );
  }
      ...
      body: BlocBuilder<PickingBloc, PickingState>(
        builder: (context, state) {
          if (state is PickingSyncing) {
            return AppLoading.centered(message: 'データ同期中...');
          }
  ...
  Widget _infoCard(PickingLine line) {
    return Container(
      decoration: BoxDecoration(
        color: AppColors.settingsColor3.withAlpha(0x14), // 8% orange tint
        borderRadius: BorderRadius.circular(8),
        border: Border.all(
          color: AppColors.settingsColor3.withAlpha(0x40), // ~25% orange
        ),
      ),
      padding: const EdgeInsets.all(12),
      child: Column(
  ...
  Widget _navigationButtons(PickingLine line) {
    ...
    final nextLabel = isLast ? (allDone ? '完了・送信' : '完了確認') : '次へ';
    final nextIcon = isLast
        ? (allDone ? Icons.cloud_upload : Icons.check)
        : Icons.chevron_right;
    final VoidCallback nextOnPressed = isLast
        ? (allDone ? _showSyncDialog : _handleLineComplete)
        : _goNext;
    final useFilled = isLast && allDone;
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: _isFocused
            ? [
                BoxShadow(
                  color: AppColors.settingsColor3.withAlpha(0x1A), // ~10%
                  blurRadius: 4,
      ...
      child: Row(
        children: [
          Icon(
            widget.icon,
            color: _isFocused ? AppColors.settingsColor3 : AppColors.gray,
          ),
          const SizedBox(width: 8),
          Expanded(
            child: TextField(
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: _isFocused
            ? [
                BoxShadow(
                  color: AppColors.settingsColor3.withAlpha(0x1A),
                  blurRadius: 4,
                  offset: const Offset(0, 1),
                )
              ]
